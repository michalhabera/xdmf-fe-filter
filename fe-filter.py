## execfile("/home/mffmh/gsoc17/xdmf-fe-filter/fe-filter.py")
...
def make_outcell_from_triangle(in_vals, element_type,
    ...
    if element_type == "DG1" or element_type == "CG1":
        ...
        scnt = ds_out.GetPoints().GetNumberOfPoints()
        for ix in range(0, 6):
            if in_cell.GetCellType() == vtk.VTK_QUADRATIC_TRIANGLE:
                c = cells_points.GetPoint(ix)
            else:
                if ix < 3:
                    c = cells_points.GetPoint(ix)
...
def make_outcell_from_incell(in_vals, in_cell, element_type,
    ...
    if in_cell_type == vtk.VTK_TRIANGLE or in_cell_type == vtk.VTK_QUADRATIC_TRIANGLE:
...
def traverse(ds_in, ds_out):
    ...
    assert in_idx_name.split("_")[-1] == "dofs"
